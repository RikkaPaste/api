...
const tixianList = db.sequelize.import("../models/tixianlist");
...
module.exports={
    list: async function(req,res,next){
        ...
        let wxopenid=req.body.wxopenid?req.body.wxopenid:"";
        //当前页之前的所有记录
        ...
        let data = await tixianList.findAndCountAll({
            where,
    ...
    add: async function(req,res,next){
        let addResult=null;
        try {
            addResult = await tixianList.create(req.body);
        } catch (error) {
            console.log(error);
        }
    ...
    findById: async function(req,res,next){
        ...
        //返回的是对象数组，里面只有一个对象
        let data = await tixianList.findAll({where:{id:paramId}});
        res.json({"code":200,"data":data,"msg":"根据id查询成功"});
        return false;
    },
    upd: async function(req,res,next){
        //返回的是受影响的行数，如:[1]
        let data = await tixianList.update(req.body,{where:{id:req.body.id}});
        res.json({"code":200,"data":data,"msg":"修改成功"});
        return false;
    },
    del: async function(req,res,next){
        let data = await tixianList.destroy({where:{id:req.body.id}});
