...
const express = db.sequelize.import("../models/express");
module.exports={
    list: async function(req,res,next){
        ...
        let data = await express.findAndCountAll({
    ...
    add: async function(req,res,next){
        let addResult=null;
        try {
            addResult = await express.create(req.body);
        } catch (error) {
            console.log(error);
        }
    ...
    findById: async function(req,res,next){
        ...
        //返回的是对象数组，里面只有一个对象
        let data = await express.findAll({where:{id:paramId}});
    ...
    upd: async function(req,res,next){
        ...
        //返回的是受影响的行数，如:[1]
        let data = await express.update(req.body,{where:{id:req.body.id}});
        res.json({"code":200,"data":data,"msg":"修改用户成功"});
        return false;
    },
    del: async function(req,res,next){
        let data = await express.destroy({where:{id:req.body.id}});
